...
async function buscarDadosFocus() {
  try {
    ...
    registrarErro("Erro ao buscar dados do Focus: " + e.message);
    preencherTabelaPremissas();
  }
}

function preencherTabelaPremissas(ipca = [3.7, 3.6, 3.5, 3.5], cdi = [10.25, 9.5, 9.0, 8.75]) {
  ...
  anos.forEach((ano, i) => {
    ...
    row.innerHTML = `
      <td>${ano}</td>
      <td><input type="number" class="ipca-input" data-ano="${ano}" value="${ipca[i] || ipca[ipca.length - 1]}" step="0.01"></td>
      <td><input type="number" class="cdi-input" data-ano="${ano}" value="${cdi[i] || cdi[cdi.length - 1]}" step="0.01"></td>
    `;
